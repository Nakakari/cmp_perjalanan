...
<script type="text/javascript">
    ...
    const table = $("#penjualan-dt").DataTable({
        ...
        "scrollX": true,
        "ajax": {
            url: "{{ url('') }}/inv_pelanggan/{{ base64_encode($id_pelanggan) }}",
            type: "POST",
            data: function(d) {
                d._token = "{{ csrf_token() }}",
    ...
    function prosesData(id_pelanggan) {
        ...
        if (sum != 0) {
            window.location.href = '/make_invoice/' + btoa(id_pelanggan) + '?sum=' + btoa(sum) + '&id=' + btoa(
                all_id_pengiriman)
        } else {
            warning_noti()
